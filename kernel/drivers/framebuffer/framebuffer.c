...
int framebuffer_gradient(void) {
	...
	// First call do a vertical gradient
	static int verticalGradient = 1;
	...
	if(verticalGradient){
		...
		for(x = 0; x < 800; x++){
			...
			int distance = 600 / 255;

			int y = 0;
			for(y = 0; y < 600 && clr > 0; y+=distance){
				framebuffer_vline(clr,y,y+distance,x);
				clr -= 1;
			}
			framebuffer_vline(clr,y,600,x);
		}
		...
		for(x = 0; x < 800; x++){
			...
			int y = 0;
			for(y = 0; y < 600 && clr > 0; y+=distance){
				framebuffer_hline(clr,y,y+distance,x);
				clr -= 1;
			}
			framebuffer_hline(clr,y,600,x);
		}
